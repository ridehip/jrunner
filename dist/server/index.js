...
const runs = new Map();
async function readConfig(confPath) {
    try {
...
function normalizeConfig(confJson) {
    if (!confJson.pannels) {
...
async function writePackage(packageJsonPath, json) {
...
app.put("/api/custom-scripts", async (req, res) => {
    try {
        ...
        const index = scripts.findIndex((script) => script.name === originalName);
        if (index === -1) {
...
app.delete("/api/custom-scripts", async (req, res) => {
    try {
        ...
        let confJson = await readConfig(confPath);
        confJson = normalizeConfig(confJson);
        confJson.pannels.customScripts = confJson.pannels.customScripts.filter((script) => script.name !== name);
        await fs.writeFile(confPath, JSON.stringify(confJson, null, 2));
        res.json({ customScripts: confJson.pannels.customScripts });
    }
    catch (error) {
